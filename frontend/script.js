...
async function getBotResponse(userMessage) {
    try {
        ...
        // Agar Spring Boot se 200 OK ke alawa koi error status aaye
        if (!response.ok) {
            const errorData = await response.json();
            console.error("Backend Error Response:", errorData);
            appendMessage("bot", "Backend Error: API Key check karo ya server logs dekho.");
            return;
        }
        ...
        if (data.reply) {
            ...
            appendMessage("bot", "No response received from AI.");
        }

    } catch (error) {
        console.error("Network/Server Error:", error);
        appendMessage("bot", "Server offline hai ya fetch request fail ho gayi.");
    }
}
});
...
async function getBotResponse(userMessage) {
    try {
        ...
        appendMessage("bot", "Spring Boot server offline hai ya connect nahi ho raha!");
    }
}
});